    ...
    <div class="nav-right col-8 pull-right right-header p-0">
      <ul class="nav-menus">
        ...
        <li class="onhover-dropdown">
          <div class="notification-box"><i data-feather="bell"> </i> @if ($nr_notifications > 0) <span class="badge rounded-pill badge-secondary">{{ $nr_notifications }} </span> @endif</div>
          <ul class="notification-dropdown onhover-show-div">
            <li>
              ...
              <div class="row">
                ...
                <div class="col-md-2">
                    @if ($nr_notifications > 0) <a href="{{ route('notificacao.read.all') }}" class="h6 f-right" data-bs-toggle="tooltip" data-bs-placement="top" title="Marcas todas como lidas"> <i class="fa fa-check-square-o"></i></a> @endif
                </div>
              </div>
            </li>
            @foreach ($notifications as $notification)
                <li>
                    <p><i class="fa fa-circle-o me-3 font-primary"> </i>{{ $notification->data['name'] }} <span class="pull-right">{{(date('Y-m-d', strtotime($notification->created_at)) == date('Y-m-d')) ? date('H:i', strtotime($notification->created_at)) : date('d/m', strtotime($notification->created_at)) }}</span></p>
                </li>
            @endforeach
            @if ($nr_notifications > 0)
                <li><a class="btn btn-primary" href="{{ route('notificacao.list') }}">Ver todas notificações</a></li>
            @else
        ...
        <li class="profile-nav onhover-dropdown p-0 me-0">
          <div class="media profile-media">
            <img class="b-r-10" src="{{asset('assets/images/dashboard/user.jpg')}}" alt="">
            <div class="media-body">
              <span>{{ $user->name }}</span>
              <p class="mb-0 font-roboto">{{ $user->nome }} <i class="middle fa fa-angle-down"></i></p>
            </div>
          </div>
          <ul class="profile-dropdown onhover-show-div">
            <li><a href="{{ route('utilizador.profile.update', Crypt::encrypt(Auth::user()->id)) }}"><i data-feather="user"></i><span>Perfil </span></a></li>
            {{-- <li><a href="#"><i data-feather="mail"></i><span>Inbox</span></a></li>
            <li><a href="#"><i data-feather="file-text"></i><span>Taskboard</span></a></li> --}}
            @if ($user->nome == 'Administradores') <li><a href="{{ route('definicoes.show') }}"><i data-feather="settings"></i><span>Definições</span></a></li> @endif
