   ...
   <div class="row">
      <div class="col-sm-12">
         <div class="card">
            ...
            <form class="theme-form mega-form" action="{{ route('viatura.alocar.put', Crypt::encrypt($alocacao->id)) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="card-body">
                        <div class="row">
                            <div class="mb-3 col-md-3">
                                ...
                                <select class="js-example-basic-single col-sm-12" name="viatura" id="viatura" onchange="javascript:getViaturaInfo(this.value)">
                                    ...
                                    @foreach ($viaturas as $viatura)
                                        <option value="{{$viatura->id}}" {{($alocacao->viatura_id == $viatura->id) ? 'selected' : ''}}>{{$viatura->matricula}}</option>
                                    @endforeach
                                </select>
                                @error('viatura') <span style="color: red">{{$message}}.</span> @enderror
                            </div>
                            <div class="mb-3 col-md-3">
                               <label class="col-form-label">Kilometragem actual (litros)  <i class="fa fa-spin fa-spinner" id="loader_odometro" style="display: none"></i></label>
                               <input class="form-control" type="text" name="odometro" id="odometro" value="{{ $alocacao->odometro }}" placeholder="XYZ km" readonly>
                            ...
                            <div class="mb-3 col-md-3">
                               ...
                               <input class="form-control" type="text" name="combustivel" id="combustivel" value="{{ $alocacao->combustivel_disponivel }}" placeholder="XY l" readonly>
                            ...
                            <div class="mb-3 col-md-3">
                               ...
                               <input class="form-control" type="number" name="distancia" id="distancia" min="1" value="{{ $alocacao->distancia_estimativa }}" placeholder="XYZ km" oninput="javascript:calculateDistance(this.value)">
                            ...
                            <div class="mb-3 col-md-3">
                               ...
                               <input class="form-control" type="text" name="estimativa_combustivel" id="estimativa_combustivel" value="{{ $alocacao->combustivel_estimativa }}" placeholder="XY l" readonly>
                               <small style="color: red">Estimativa</small>
                            </div>
                            <div class="mb-3 col-md-3">
                                ...
                                <select class="js-example-basic-single col-sm-12" name="motorista">
                                    ...
                                    @foreach ($motoristas as $motorista)
                                        <option value="{{$motorista->id}}" {{($alocacao->motorista_id == $motorista->id) ? 'selected' : ''}}>{{$motorista->nome . ' ' . $motorista->apelido}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <select class="js-example-basic-multiple col-sm-12" name="rota[]" multiple="multiple">
                                    @foreach ($rotas as $rota)
                                        <option value="{{$rota->id}}" {{ (in_array($rota->id, $rotas_motorista)) ? 'selected' : '' }} >{{$rota->name}}</option>
                                    @endforeach
                                </select>
        ...
        if (id) {
            ...
            $.ajax({
                    ...
                    success: function(data) {
                        estimativa = parseFloat(data['fuel_efficiency']) * parseFloat(distance)
